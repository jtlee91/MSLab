...
interface CageGridProps {
  ...
  onCageClick: (cage: Cage) => void;
}

export default function CageGrid({ gridData, onCageClick }: CageGridProps) {
  ...
  return (
    <div className={styles.gridContainer}>
      <div className={styles.gridInfo}>
        ...
        <span className={styles.gridInfo__hint}>클릭으로 케이지를 배정/해제할 수 있습니다</span>
      </div>
      <div className={styles.grid} style={gridStyle} role="grid" aria-label="케이지 그리드">
        {sortedCages.map((cage) => (
          <CageCell key={cage.id} cage={cage} onClick={onCageClick} />
        ))}
      </div>
    </div>
  );
