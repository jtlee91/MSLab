...
| ID | 사용자 스토리 | 인수 조건 | 상태 |
|----|---------------|-----------|------|
| US-1.1 | 연구원으로서, 빈 케이지를 클릭하면 교수 목록이 바로 뜨고, 선택 즉시 배정되길 원한다 | **클릭/터치** → 교수 목록 → 선택 즉시 저장 (확인 버튼 없음) | ✅ 구현 |
| US-1.2 | 연구원으로서, 사용 중인 케이지를 클릭하면 해제할 수 있길 원한다 | 클릭 → "사용 종료?" 확인/취소 팝업 → 확인 시 즉시 해제 | ✅ 구현 |
| US-1.3 | 연구원으로서, 랙1/2/3을 탭으로 쉽게 전환하고 싶다 | 스프레드시트 Sheet 탭처럼 전환 (가로 스크롤 지원) | ✅ 구현 |
| US-1.4 | 관리자로서, 랙의 그리드 크기(예: 8×8)를 설정하고 싶다 | 설정 화면에서 랙별 행/열 수 지정 가능 (크기 확대/축소) | ✅ 구현 |
| US-1.5 | 관리자로서, 교수 정보(이름, 담당 학생, 연락처)를 등록/관리하고 싶다 | 설정 > 교수 관리 페이지에서 CRUD | ✅ 구현 |

> **변경 사항**: US-1.1의 "더블클릭" → "단일 클릭"으로 변경 (모바일 UX 개선)

### 4.2 FEAT-2: 실시간 현황판 (✅ 구현 완료)

| ID | 사용자 스토리 | 인수 조건 | 상태 |
|----|---------------|-----------|------|
| US-2.1 | 관리자로서, 3개 랙의 사용/미사용 케이지 수를 한눈에 보고 싶다 | 대시보드 상단에 요약 카드 | ✅ 구현 |
| US-2.2 | 관리자로서, 교수별 현재 사용 중인 케이지 수를 보고 싶다 | 교수별 사용 현황 리스트 | ✅ 구현 |
| US-2.3 | 관리자로서, 일/주/월별 교수별 비용 차트를 보고 싶다 | 대시보드 하단에 비용 차트 | ✅ 구현 |

### 4.3 FEAT-3: 과금 리포트 (✅ 구현 완료)

| ID | 사용자 스토리 | 인수 조건 | 상태 |
|----|---------------|-----------|------|
| US-3.1 | 관리자로서, 교수별 비용 요약을 xlsx로 다운로드하고 싶다 | [요약] 시트: 교수명, 케이지 수, 총 비용 | ✅ 구현 |
| US-3.2 | 관리자로서, 일별 상세 사용 내역을 xlsx로 다운로드하고 싶다 | [상세] 시트: 날짜, 케이지ID, 교수, 비용 | ✅ 구현 |
